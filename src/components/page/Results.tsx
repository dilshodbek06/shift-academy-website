      ...
      <Container className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-20 max-w-5xl mx-auto">
          ...
          >
            <div className="absolute top-0 right-0 p-8 opacity-20 group-hover:opacity-30 transition-opacity group-hover:scale-110 duration-500">
               <Briefcase className="w-24 h-24 text-brand" />
            </div>
        ...
        >
            ...
            <div className="relative w-full overflow-hidden mt-10 mb-6">
              ...
              <div className="absolute inset-y-0 left-0 w-24 md:w-48 bg-gradient-to-r from-[#F8FAFC] to-transparent z-10" />
              <div className="absolute inset-y-0 right-0 w-24 md:w-48 bg-gradient-to-l from-[#F8FAFC] to-transparent z-10" />

              <div className="flex flex-col gap-6 w-[200vw] sm:w-[150vw] xl:w-full ml-[-50vw] sm:ml-[-25vw] xl:ml-0">
                 {/* Row 1 - Moves Left */}
                 <div className="flex overflow-hidden relative w-full">
                    ...
                    >
                      {[...companies, ...companies].map((company, idx) => (
                        <div 
                          key={`row1-${idx}`}
                          className="h-24 md:h-28 w-56 md:w-64 bg-white border border-slate-100 rounded-3xl flex items-center justify-center px-8 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-brand/30 hover:text-brand transition-all duration-300 group cursor-default"
                        >
                          <span className="font-extrabold text-2xl md:text-3xl text-slate-400 group-hover:text-slate-800 transition-colors tracking-tight line-clamp-1">{company}</span>
                        </div>
                      ))}
                    </motion.div>
                 </div>
